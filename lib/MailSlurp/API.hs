...
type MailSlurpAPI
    ...
    :<|> "attachments" :> QueryParam "page" Int :> QueryParam "size" Int :> QueryParam "sort" Text :> Verb 'GET 200 '[JSON] PageAttachmentEntity -- 'getAttachments' route
    ...
    :<|> "webhooks" :> "test" :> Verb 'GET 200 '[JSON] WebhookPayload -- 'getTestWebhookPayload' route
...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , getAttachments :: Maybe Int -> Maybe Int -> Maybe Text -> m PageAttachmentEntity{- ^ Get all attachments in paginated response. Each entity contains meta data for the attachment such as `name` and `content-type`. Use the `attachmentId` and the download endpoints to get the file contents. -}
  ...
  , getTestWebhookPayload :: m WebhookPayload{- ^  -}
