...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , getAllWebhooks :: Maybe Int -> Maybe Int -> Maybe Text -> Maybe Text -> Maybe UTCTime -> Maybe UUID -> Maybe UUID -> Maybe UUID -> Maybe UTCTime -> Maybe Text -> Maybe Text -> Maybe Text -> m PageWebhookProjection{- ^ List webhooks in paginated form. Allows for page index, page size, and sort direction. -}
