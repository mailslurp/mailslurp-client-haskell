...
type MailSlurpAPI
    ...
    :<|> "waitForMatchingEmails" :> QueryParam "count" Int :> QueryParam "delay" Integer :> QueryParam "inboxId" UUID :> QueryParam "since" UTCTime :> QueryParam "sort" Text :> QueryParam "timeout" Integer :> QueryParam "unreadOnly" Bool :> ReqBody '[JSON] MatchOptions :> Verb 'POST 200 '[JSON] [EmailPreview] -- 'waitForMatchingEmail' route
...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , markAsRead :: UUID -> Maybe Bool -> m EmailPreview{- ^ Marks an email as read or unread. Pass boolean read flag to set value. -}
