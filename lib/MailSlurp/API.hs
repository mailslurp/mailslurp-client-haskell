...
type MailSlurpAPI
    ...
    :<|> "internal" :> "saml" :> "user" :> QueryParam "key" Text :> ReqBody '[JSON] GetOrCreateSamlUserOptions :> Verb 'POST 200 '[JSON] UserDto -- 'getSamlUserOrCreate' route
    ...
    :<|> "attachments" :> "bytes" :> QueryParam "filename" Text :> ReqBody '[JSON] [Text] :> Header "contentType" Text :> Verb 'POST 200 '[JSON] [Text] -- 'uploadAttachmentBytes' route
    ...
    :<|> "createInbox" :> QueryParam "allowTeamAccess" Bool :> QueryParam "useDomainPool" Bool :> QueryParam "expiresAt" UTCTime :> QueryParam "expiresIn" Integer :> QueryParam "emailAddress" Text :> QueryParam "inboxType" Text :> QueryParam "description" Text :> QueryParam "name" Text :> QueryParam "tags" (QueryList 'MultiParamArray (Text)) :> QueryParam "favourite" Bool :> Verb 'POST 200 '[JSON] InboxDto -- 'createRandomInbox' route
...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , getSamlUserOrCreate :: Maybe Text -> GetOrCreateSamlUserOptions -> m UserDto{- ^  -}
  ...
  , uploadAttachmentBytes :: Maybe Text -> [Text] -> Maybe Text -> m [Text]{- ^  -}
  ...
  , createRandomInbox :: Maybe Bool -> Maybe Bool -> Maybe UTCTime -> Maybe Integer -> Maybe Text -> Maybe Text -> Maybe Text -> Maybe Text -> Maybe [Text] -> Maybe Bool -> m InboxDto{- ^ Returns an Inbox with an `id` and an `emailAddress` -}
