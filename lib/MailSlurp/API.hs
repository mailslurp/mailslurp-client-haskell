...
type MailSlurpAPI
    ...
    :<|> "user" :> "smtp" :> "password" :> Verb 'GET 200 '[JSON] Text -- 'getSmtpPassword' route
    :<|> "user" :> "smtp" :> "username" :> Verb 'GET 200 '[JSON] Text -- 'getSmtpUsername' route
    ...
    :<|> "emails" :> QueryParam "useDomainPool" Bool :> QueryParam "virtualSend" Bool :> ReqBody '[JSON] SendEmailOptions :> Verb 'POST 200 '[JSON] () -- 'sendEmailSourceOptional' route
    ...
    :<|> "phone" :> "numbers" :> QueryParam "page" Int :> QueryParam "size" Int :> QueryParam "sort" Text :> QueryParam "since" UTCTime :> QueryParam "before" UTCTime :> Verb 'GET 200 '[JSON] PagePhoneNumberProjection -- 'getPhoneNumbers' route
    ...
    :<|> "sent" :> "delivery-status" :> Capture "deliveryId" UUID :> Verb 'GET 200 '[JSON] DeliveryStatus -- 'getSentDeliveryStatus' route
    ...
    :<|> "sms" :> Capture "smsId" UUID :> Verb 'GET 200 '[JSON] SmsMessage -- 'getSmsMessage' route
...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , getSmtpPassword :: m Text{- ^  -}
  , getSmtpUsername :: m Text{- ^  -}
  ...
  , sendEmailSourceOptional :: Maybe Bool -> Maybe Bool -> SendEmailOptions -> m (){- ^ Alias for `InboxController.sendEmail` method - see original method for full details. Sends an email from a given inbox that you have created. If no inbox is supplied a random inbox will be created for you and used to send the email. -}
  ...
  , getMissedEmail :: UUID -> m MissedEmail{- ^ List emails that were missed due to plan limits. -}
  ...
  , getPhoneNumbers :: Maybe Int -> Maybe Int -> Maybe Text -> Maybe UTCTime -> Maybe UTCTime -> m PagePhoneNumberProjection{- ^  -}
  ...
  , getSentDeliveryStatus :: UUID -> m DeliveryStatus{- ^ Get a sent email delivery status -}
  ...
  , getSmsMessage :: UUID -> m SmsMessage{- ^ Returns a SMS summary object with content. -}
...
createMailSlurpClient = MailSlurpBackend{..}
  where
    ((coerce -> createAlias) :<|>
     ...
     (coerce -> updateAlias) :<|>
     (coerce -> getSmtpPassword) :<|>
     (coerce -> getSmtpUsername) :<|>
     (coerce -> getUserInfo) :<|>
