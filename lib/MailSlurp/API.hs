...
type MailSlurpAPI
    ...
    :<|> "inboxes" :> Capture "inboxId" UUID :> "rulesets" :> ReqBody '[JSON] CreateInboxRulesetOptions :> Verb 'POST 200 '[JSON] () -- 'createInboxRuleset' route
    ...
    :<|> "inboxes" :> Capture "inboxId" UUID :> "rulesets" :> QueryParam "page" Int :> QueryParam "searchFilter" Text :> QueryParam "size" Int :> QueryParam "sort" Text :> Verb 'GET 200 '[JSON] () -- 'listInboxRulesets' route
    ...
    :<|> "missed-emails" :> "waitForNthMissedEmail" :> QueryParam "index" Int :> Verb 'GET 200 '[JSON] MissedEmail -- 'waitForNthMissedEmail' route
    ...
    :<|> "tracking" :> "pixels" :> QueryParam "page" Int :> QueryParam "size" Int :> QueryParam "sort" Text :> Verb 'GET 200 '[JSON] PageTrackingPixelProjection -- 'getAllTrackingPixels' route
    ...
    :<|> "webhooks" :> "results" :> Capture "webhookResultId" UUID :> Verb 'GET 200 '[JSON] WebhookResultEntity -- 'getWebhookResult' route
...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , forwardEmail :: UUID -> ForwardEmailOptions -> m (){- ^ Forward an existing email to new recipients. The sender of the email will be the inbox that received the email you are forwarding. You can override the sender with the `from` option. Note you must have access to the from address in MailSlurp to use the override. For more control consider fetching the email and sending it a new using the send email endpoints. -}
  ...
  , createInboxRuleset :: UUID -> CreateInboxRulesetOptions -> m (){- ^ Create a new inbox rule for forwarding, blocking, and allowing emails when sending and receiving -}
  ...
  , listInboxRulesets :: UUID -> Maybe Int -> Maybe Text -> Maybe Int -> Maybe Text -> m (){- ^ List all rulesets attached to an inbox -}
  ...
  , waitForNthMissedEmail :: Maybe Int -> m MissedEmail{- ^ Wait for 0 based index missed email -}
  ...
  , createTrackingPixel :: CreateTrackingPixelOptions -> m TrackingPixelDto{- ^ Create a tracking pixel -}
  , getAllTrackingPixels :: Maybe Int -> Maybe Int -> Maybe Text -> m PageTrackingPixelProjection{- ^ List tracking pixels in paginated form -}
  ...
  , getWebhookResult :: UUID -> m WebhookResultEntity{- ^  -}
