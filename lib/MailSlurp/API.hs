...
type MailSlurpAPI
    ...
    :<|> "phone" :> "numbers" :> Capture "phoneNumberId" UUID :> "test" :> ReqBody '[JSON] TestPhoneNumberOptions :> Verb 'POST 200 '[JSON] () -- 'testPhoneNumberSendSms' route
...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , testPhoneNumberSendSms :: UUID -> TestPhoneNumberOptions -> m (){- ^  -}
