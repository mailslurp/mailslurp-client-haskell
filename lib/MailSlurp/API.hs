...
type MailSlurpAPI
    ...
    :<|> "forwarders" :> "events" :> QueryParam "page" Int :> QueryParam "size" Int :> QueryParam "sort" Text :> Verb 'GET 200 '[JSON] PageInboxForwarderEvents -- 'getAllInboxForwarderEvents' route
...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , checkEmailBody :: UUID -> m CheckEmailBodyResults{- ^ Find dead links, broken images, and spelling mistakes in email body. Will call included links via HTTP so do not invoke if your links are sensitive or stateful. -}
  ...
  , getAllInboxForwarderEvents :: Maybe Int -> Maybe Int -> Maybe Text -> m PageInboxForwarderEvents{- ^ Get all inbox forwarder events -}
