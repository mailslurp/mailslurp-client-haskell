...
type MailSlurpAPI
    ...
    :<|> "bounce" :> "emails" :> Capture "id" UUID :> Verb 'GET 200 '[JSON] Bounce -- 'getBouncedEmail' route
    ...
    :<|> "bounce" :> "recipients" :> Capture "id" UUID :> Verb 'GET 200 '[JSON] BounceRecipient -- 'getBouncedRecipient' route
...
data MailSlurpBackend m = MailSlurpBackend
  ...
  , getBouncedEmail :: UUID -> m Bounce{- ^ Bounced emails are email you have sent that were rejected by a recipient -}
  ...
  , getBouncedRecipient :: UUID -> m BounceRecipient{- ^ Bounced emails are email you have sent that were rejected by a recipient -}
