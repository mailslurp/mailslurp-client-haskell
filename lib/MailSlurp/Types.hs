...
-- | 
data Bounce = Bounce
  { bounceBounceMta :: Maybe Text -- ^ 
  , bounceBounceRecipients :: Maybe [Text] -- ^ 
  , bounceBounceSubType :: Maybe Text -- ^ 
  , bounceBounceType :: Maybe Text -- ^ 
  , bounceCreatedAt :: UTCTime -- ^ 
  , bounceId :: Maybe UUID -- ^ 
  , bounceNotificationType :: Text -- ^ 
  , bounceSender :: Text -- ^ 
  , bounceSentToRecipients :: Maybe [Text] -- ^ 
  , bounceUpdatedAt :: UTCTime -- ^ 
  , bounceUserId :: UUID -- ^ 
  } deriving (Show, Eq, Generic, Data)

instance FromJSON Bounce where
  parseJSON = genericParseJSON (removeFieldLabelPrefix True "bounce")
instance ToJSON Bounce where
  toJSON = genericToJSON (removeFieldLabelPrefix False "bounce")
...
data WebhookResultEntity = WebhookResultEntity
  ...
  , webhookResultEntityResultType :: Maybe Text -- ^ 
