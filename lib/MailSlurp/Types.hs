...
data CreateDomainOptions = CreateDomainOptions
  ...
  , createDomainOptionsDomainType :: Maybe Text -- ^ Domain type to create. HTTP or SMTP domain. HTTP domain uses MailSlurps SES MX records. SMTP uses a custom SMTP server MX record
  } deriving (Show, Eq, Generic, Data)
...
data CreateInboxDto = CreateInboxDto
  ...
  , createInboxDtoInboxType :: Maybe Text -- ^ HTTP or SMTP inbox
...
data PageAttachmentEntity = PageAttachmentEntity
  { pageAttachmentEntityContent :: Maybe [AttachmentEntity] -- ^ 
...
data WebhookPayload = WebhookPayload
  { webhookPayloadAttachmentMetaDatas :: Maybe [AttachmentMetaData] -- ^ List of attachment meta data objects if attachments present
  , webhookPayloadBcc :: Maybe [Text] -- ^ List of `BCC` recipients email was addressed to
  , webhookPayloadCc :: Maybe [Text] -- ^ List of `CC` recipients email was addressed to
  , webhookPayloadCreatedAt :: Maybe UTCTime -- ^ Date time of event creation
  , webhookPayloadEmailId :: Maybe UUID -- ^ ID of the email that was received. Use this ID for fetching the email
  , webhookPayloadEventName :: Maybe Text -- ^ Name of the event type webhook is being triggered for
  , webhookPayloadFrom :: Maybe Text -- ^ Who the email was sent from
  , webhookPayloadInboxId :: Maybe UUID -- ^ Id of the inbox that receive an email
  , webhookPayloadMessageId :: Maybe Text -- ^ Idempotent message ID. Store this ID locally or in a database to prevent message duplication.
  , webhookPayloadSubject :: Maybe Text -- ^ The subject line of the email message
  , webhookPayloadTo :: Maybe [Text] -- ^ List of `To` recipients that email was addressed to
  , webhookPayloadWebhookId :: Maybe UUID -- ^ ID of webhook entity being triggered
  , webhookPayloadWebhookName :: Maybe Text -- ^ Name of the webhook being triggered
  } deriving (Show, Eq, Generic, Data)

instance FromJSON WebhookPayload where
  parseJSON = genericParseJSON (removeFieldLabelPrefix True "webhookPayload")
instance ToJSON WebhookPayload where
  toJSON = genericToJSON (removeFieldLabelPrefix False "webhookPayload")
