...
data CreateInboxRulesetOptions = CreateInboxRulesetOptions
  { createInboxRulesetOptionsAction :: Maybe Text -- ^ Action to be taken when the ruleset matches an email for the given scope. For example: `BLOCK` action with target `*` and scope `SENDING_EMAILS` blocks sending to all recipients. Note `ALLOW` takes precedent over `BLOCK`.
...
data CreateWebhookOptions = CreateWebhookOptions
  ...
  , createWebhookOptionsEventName :: Maybe Text -- ^ Optional webhook event name. Default is `EMAIL_RECEIVED` and is triggered when an email is received by the inbox associated with the webhook. Payload differ according to the webhook event name. The other events are `NEW_EMAIL`, `NEW_CONTACT`, and `NEW_ATTACHMENT`.
...
data InboxRulesetProjection = InboxRulesetProjection
  { inboxRulesetProjectionAction :: Text -- ^ 
  , inboxRulesetProjectionHandler :: Text -- ^ 
  , inboxRulesetProjectionId :: UUID -- ^ 
  , inboxRulesetProjectionInboxId :: UUID -- ^ 
  , inboxRulesetProjectionScope :: Text -- ^ 
  , inboxRulesetProjectionTarget :: Text -- ^ 
  } deriving (Show, Eq, Generic, Data)

instance FromJSON InboxRulesetProjection where
  parseJSON = genericParseJSON (removeFieldLabelPrefix True "inboxRulesetProjection")
instance ToJSON InboxRulesetProjection where
  toJSON = genericToJSON (removeFieldLabelPrefix False "inboxRulesetProjection")
...
data PageInboxRulesetProjection = PageInboxRulesetProjection
  { pageInboxRulesetProjectionContent :: Maybe [InboxRulesetProjection] -- ^ 
  , pageInboxRulesetProjectionEmpty :: Maybe Bool -- ^ 
  , pageInboxRulesetProjectionFirst :: Maybe Bool -- ^ 
  , pageInboxRulesetProjectionLast :: Maybe Bool -- ^ 
  , pageInboxRulesetProjectionNumber :: Maybe Int -- ^ 
  , pageInboxRulesetProjectionNumberOfElements :: Maybe Int -- ^ 
  , pageInboxRulesetProjectionPageable :: Maybe Pageable -- ^ 
  , pageInboxRulesetProjectionSize :: Maybe Int -- ^ 
  , pageInboxRulesetProjectionSort :: Maybe Sort -- ^ 
  , pageInboxRulesetProjectionTotalElements :: Maybe Integer -- ^ 
  , pageInboxRulesetProjectionTotalPages :: Maybe Int -- ^ 
  } deriving (Show, Eq, Generic, Data)

instance FromJSON PageInboxRulesetProjection where
  parseJSON = genericParseJSON (removeFieldLabelPrefix True "pageInboxRulesetProjection")
instance ToJSON PageInboxRulesetProjection where
  toJSON = genericToJSON (removeFieldLabelPrefix False "pageInboxRulesetProjection")
