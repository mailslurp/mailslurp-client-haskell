...
-- | 
data GetOrCreateSamlUserOptions = GetOrCreateSamlUserOptions
  { getOrCreateSamlUserOptionsIssuer :: Text -- ^ 
  , getOrCreateSamlUserOptionsNameId :: Text -- ^ 
  , getOrCreateSamlUserOptionsNameIdFormat :: Maybe Text -- ^ 
  } deriving (Show, Eq, Generic, Data)

instance FromJSON GetOrCreateSamlUserOptions where
  parseJSON = genericParseJSON (removeFieldLabelPrefix True "getOrCreateSamlUserOptions")
instance ToJSON GetOrCreateSamlUserOptions where
  toJSON = genericToJSON (removeFieldLabelPrefix False "getOrCreateSamlUserOptions")
...
data UserDto = UserDto
  { userDtoId :: UUID -- ^ 
  , userDtoApiKey :: Text -- ^ 
  , userDtoEmailAddress :: Text -- ^ 
  , userDtoEmailAddressMd5 :: Text -- ^ 
  , userDtoCreated :: Maybe UTCTime -- ^ 
  , userDtoUserType :: Maybe Text -- ^ 
  , userDtoOrganization :: Maybe UUID -- ^ Does user belong to an organization
  , userDtoVerified :: Maybe Text -- ^ Has user accepted an organization invite
  , userDtoHasPassword :: Bool -- ^ 
  , userDtoIsFrozen :: Bool -- ^ 
  , userDtoAddNewContacts :: Maybe Bool -- ^ 
  , userDtoSsoProvider :: Maybe Text -- ^ 
  , userDtoCustomerId :: Maybe Text -- ^ 
  , userDtoHasOnboarded :: Maybe Bool -- ^ 
  , userDtoImapUsername :: Maybe Text -- ^ 
  , userDtoImapPassword :: Maybe Text -- ^ 
  , userDtoSmtpUsername :: Maybe Text -- ^ 
  , userDtoSmtpPassword :: Maybe Text -- ^ 
  } deriving (Show, Eq, Generic, Data)

instance FromJSON UserDto where
  parseJSON = genericParseJSON (removeFieldLabelPrefix True "userDto")
instance ToJSON UserDto where
  toJSON = genericToJSON (removeFieldLabelPrefix False "userDto")
